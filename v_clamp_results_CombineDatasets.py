...
# =============================================================================
# ROOT DIRECTORIES  –  key = culture label, value = path
# =============================================================================

roots = {
    "Mono": "/Volumes/BWH-HVDATA/Individual Folders/Garrett Scarpa/Ephys/Patch and LFP/Analyses/Patch/RD_SCLC_TumorCells_MonoCulture",
    "Co":   "/Volumes/BWH-HVDATA/Individual Folders/Garrett Scarpa/Ephys/Patch and LFP/Analyses/Patch/RD_SCLC_TumorCells_CoCulture",
}

# Display names used in axis labels / titles
culture_labels = {
    "Mono": "MonoCulture",
    "Co":   "CoCulture",
}
...
# =============================================================================
# LOAD EVENTS  –  iterate over both roots
# =============================================================================
...
# condition keys are tuples: (folder_name, culture_key)
# e.g.  ("DMSO", "Mono"),  ("DMSO", "Co")

for culture_key, root in roots.items():
    ...
    print(f"\n[{culture_key}] Found {len(event_files)} saved event files")
    ...
    for event_file in event_files:
        ...
        condition   = (folder_name, culture_key)          # composite key
        ...
        for (rec_path, rec_file), rec_events in rec_groups.items():
            ...
            for e in rec_events:
                ...
                # ── DEBUG ────────────────────────────────────────────────────
                sum_ttp_rec  = ttp + rec
                residual_dbg = right_t_ms - sum_ttp_rec
                if abs(residual_dbg) > 0.5:
                    print(
                        f"[RESIDUAL DEBUG] {condition} | {rec_file}\n"
                        f"  right_t_ms = {right_t_ms:.4f} ms  TTP = {ttp:.4f}  Rec = {rec:.4f}\n"
                        f"  residual   = {residual_dbg:.4f} ms  ← should be ~0"
                    )
                # ── END DEBUG ─────────────────────────────────────────────────
...
# –  interleaved: CondA-Mono, CondA-Co, CondB-Mono, CondB-Co …
# =============================================================================

# Collect all folder names (sorted) and all culture keys (in roots order)
all_folder_names = sorted({k[0] for k in event_features.keys()})
all_culture_keys = list(roots.keys())          # preserves insertion order

# Interleaved ordering
conditions_ordered = [
    (fn, ck)
    for fn in all_folder_names
    for ck in all_culture_keys
    if (fn, ck) in event_features
]

# Readable labels for x-axis ticks
def cond_label(fn, ck):
    return f"{fn}\n({culture_labels[ck]})"

tick_labels = [cond_label(fn, ck) for fn, ck in conditions_ordered]

condition_N = {c: len(condition_recordings[c]) for c in conditions_ordered}
...
feature_summary = {k: [] for k in (
    "condition", "frequency", "frequency_sem",
...
raw_event_features = event_features


# =============================================================================
# COLOR PALETTE
# –  same hue family per folder_name; lighter shade for Co-culture
# =============================================================================

# Base hues per folder (up to 4 folders)
_base_hues = ["#1d1d1d", "#7B2CBF", "#2196F3", "#FF5722"]
_co_hues   = ["#888888", "#C77DFF", "#64B5F6", "#FF8A65"]

folder_to_base = {fn: _base_hues[i % len(_base_hues)]
                  for i, fn in enumerate(all_folder_names)}
folder_to_co   = {fn: _co_hues[i % len(_co_hues)]
                  for i, fn in enumerate(all_folder_names)}

def cond_color(fn, ck):
    return folder_to_base[fn] if ck == "Mono" else folder_to_co[fn]

colors = [cond_color(fn, ck) for fn, ck in conditions_ordered]
...
# Separator lines between interleaved pairs (after every 2nd bar)
pair_sep_positions = [i - 0.5 for i in range(2, n_conds, 2)]
...
for col_i, (key, label, sem_key) in enumerate(zip(keys, labels, sem_keys)):
    ...
    ax.bar(x, means, color=colors, alpha=0.85,
           width=0.60, zorder=2, linewidth=0.6, edgecolor="white")

    ax.errorbar(x, means, yerr=sems,
                fmt="none", color="black",
                capsize=3, capthick=0.8, elinewidth=0.8, zorder=3)

    for xi, cond, color in zip(x, conditions_ordered, colors):
        fn, ck = cond
        if key == "frequency":
    ...
    # for sep_x in pair_sep_positions:
    #     ax.axvline(sep_x, color="#cccccc", lw=0.7, ls="--", zorder=1)

    # # Significance brackets (all pairwise Welch t-tests)
    # data_by_cond = []
    # for cond in conditions_ordered:
    #     fn, ck = cond
    #     if key == "frequency":
    ...
    #     for pair_i, (i, j) in enumerate(combinations(range(n_conds), 2)):
...
bb_freq  = ax_pos(top_axes[0])
bb_left  = ax_pos(top_axes[1])
bb_right = ax_pos(top_axes[5])
...
# ------------------------------------------------------------------
# Legend  (Mono = filled darker, Co = filled lighter)
# ------------------------------------------------------------------
legend_handles = []
for fn in all_folder_names:
    legend_handles.append(
        plt.Rectangle((0, 0), 1, 1,
                       facecolor=folder_to_base[fn], edgecolor="none",
                       label=f"{fn} (Mono)")
    )
    legend_handles.append(
        plt.Rectangle((0, 0), 1, 1,
                       facecolor=folder_to_co[fn], edgecolor="none",
                       label=f"{fn} (Co)")
    )
...
fig.legend(
    ...
    frameon=False,
    ncol=2,
    title="Condition (culture)",
    title_fontsize=7.5,
)


# ------------------------------------------------------------------
# Waveform plots  (one row per condition, interleaved order)
# ------------------------------------------------------------------
_fs_ref = None

for row_idx, cond in enumerate(conditions_ordered):
    ...
    # Use fs from any loaded trace (all share the same rate)
    if _fs_ref is None:
        _fs_ref = list(trace_cache.values())[0][2] if trace_cache else 10000

    fs_plot     = _fs_ref
    pre_samp_plt = int(window_ms_pre / 1000 * fs_plot)
    t            = (np.arange(wfs.shape[1]) - pre_samp_plt) / fs_plot * 1000

    for wf in wfs:
        ax.plot(t, wf, color=color,
                alpha=max(0.03, min(0.12, 3 / n_ev)),
                lw=0.6, rasterized=True)

    mean_wf = np.mean(wfs, axis=0)
    sem_wf  = np.std(wfs, axis=0) / np.sqrt(n_ev)
    ...
    ax.set_title(
        f"{fn}  –  {culture_labels[ck]}   "
        f"(n = {n_ev} events,  N = {condition_N[cond]} recordings)",
...
# =============================================================================
# CONFIRMATION FIGURE  (TTP + Recovery vs. Base duration)
# =============================================================================

fig2, axes2 = plt.subplots(
    1, n_conds,
    figsize=(3.2 * n_conds, 4),
    sharey=False,
)
...
for ax, cond, color in zip(axes2, conditions_ordered, colors):
    ...
    ax.set_xlim(-0.6, 2.6)
    ax.set_title(f"{fn}\n({culture_labels[ck]})", fontsize=9, fontweight="bold", pad=4)
